...
export default function InvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [invoiceData, setInvoiceData] = useState({
    invoiceNo: "",
    invoiceDate: "",
    customerName: "PLN Electricity Services",
  ...
  const addItemRow = () => {
    setInvoiceData({
      ...invoiceData,
      items: [...invoiceData.items, { description: "", span: 0, price: 0 }],
    });
  };
  ...
  // Helper untuk parsing span yang bisa jadi persentase
  const parsespan = (span: string | number) => {
    const spanStr = String(span).trim();
    if (spanStr.endsWith("%")) {
      const num = parseFloat(spanStr.replace("%", ""));
      return isNaN(num) ? 0 : num / 100;
    }
    const num = parseFloat(spanStr);
    return isNaN(num) ? 0 : num;
  };

  // Kalkulasi Otomatis Total
  const subtotal = invoiceData.items.reduce(
    (acc, item) => acc + parsespan(item.span) * Number(item.price),
    0,
  );
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex flex-col gap-2">
                      ...
                      <input
                        ...
                        placeholder="Span"
                        value={item.span}
                        onChange={(e) => handleChange(e, index, "span")}
                        className="w-full p-2 bg-[#090d16]/80 light:bg-slate-50 border border-slate-700/80 light:border-slate-300 rounded-xl text-sm text-white light:text-slate-900 focus:outline-none focus:border-brand-cyan focus:ring-1 focus:ring-brand-cyan transition-all"
                      />
                    </div>
                    <div className="flex flex-col gap-2">
            ...
            <table className="w-full border-collapse border-2 border-black text-black mb-4">
              ...
              <tbody>
                {invoiceData.items.map((item, idx) => {
                  const totalItem = parsespan(item.span) * Number(item.price);
                  return (
                    <tr key={idx}>
